					...
					<table cellspacing="0" class="listcontent">
						<tr>
							<td>
								<h1>商品目录</h1>
								<hr />
								<h1>${category}</h1>&nbsp;&nbsp;&nbsp;&nbsp;共${pageInfo.total}种商品
								<hr />
								...
								<div class="pagination">
									<ul>
										<c:if test="${pageInfo.isFirstPage==false}">
											<li class="nextPage">
												<a href="${pageContext.request.contextPath}/client/product/findProductByCategory?category=${category}&pageNum=${pageInfo.pageNum-1}">&lt;&lt;上一页</a>
											</li>
										</c:if>
										...
										<c:forEach begin="1" end="${pageInfo.pages}" var="page">
											...
											<c:if test="${page!=pageInfo.pageNum}">
												<li><a href="${pageContext.request.contextPath}/client/product/findProductByCategory?pageNum=${page}&category=${category}">${page}</a>
												</li>
											</c:if>
										</c:forEach>
										...
										<c:if test="${pageInfo.isLastPage==false}">
											<li class="nextpage">
												<a href="${pageContext.request.contextPath}/client/product/findProductByCategory?category=${category}&pageNum=${pageInfo.pageNum+1}">下一页&gt;&gt;</a>
											</li>
										</c:if>
									</ul>
								</div>
